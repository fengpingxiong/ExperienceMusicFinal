class firstFlower {
  ...
  firstFlower (String _s, float _cx, float _cy) {
    s = _s;
    len = s.length();
    pos = new PVector[len];
    radii = new float[len];
...
void firstFlowerSetup() {
  //choose the poem/song file
  words = loadStrings(wordList[fileCount % poem.length]);

  firstFlowers = new firstFlower[words.length];
  float slice = radians(360/(float)words.length);
  ...
  for (int i = 0; i < words.length; i++) {
    ...
    float xpos = width*.75 + cos(angle) * radius;
    float ypos = height/2 + sin(angle) * radius;

    firstFlowers[i] = new firstFlower(words[i], xpos, ypos);
  }
}
